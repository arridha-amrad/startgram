    ...
    <Field>
      ...
      <InputGroup>
        <div className="relative w-full">
          <div
            className="pointer-events-none absolute inset-0 border border-transparent px-2.5 py-2 text-base wrap-break-word whitespace-pre-wrap md:text-sm min-h-24"
            aria-hidden="true"
          >
            <span className="text-transparent">
              {watchedValue.slice(0, MAX_CAPTION_LENGTH)}
            </span>
            <span className="rounded-sm bg-destructive/30 text-transparent">
              {watchedValue.slice(MAX_CAPTION_LENGTH)}
            </span>
          </div>
          <InputGroupTextarea
            id={id}
            ...
            className="min-h-24 text-base w-full text-start"
          />
        </div>
        <InputGroupAddon
          align="block-end"
